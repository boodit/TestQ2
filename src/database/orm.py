...
class AsyncORM:
    ...
    async def get_player(username: str):
        async with async_session() as session:
            query = select(Player).filter_by(username=username)
    ...
    async def get_active_game_room():
        async with async_session() as session:
            query = select(Game).where(Game.game_end == False).limit(30)
            res = await session.execute(query)
            result = res.scalars().all()
            return result
    ...
    async def get_all_play_game():
        async with async_session() as session:
            query = select(Game).where(Game.game_end == False)
            res = await session.execute(query)
            result = res.scalars().all()
            return result
    ...
    @staticmethod
    async def update_board_in_game(
        game_id: int, first_player_board: list, second_player_board: list
    ):
        async with async_session() as session:
            ...
            stmt = (
                update(Game)
                .values(
                    first_player_board=json_first_player_board,
                    second_player_board=json_second_player_board,
                )
    ...
    async def update_player_status(player_id: int, status: Active = Active.out_game):
        async with async_session() as session:
            stmt = update(Player).values(active=status).where(Player.id == player_id)
            await session.execute(stmt)
            await session.commit()
            return True
